-- Configura a sessão para aceitar ponto como separador decimal e vírgula como milhar (comum em ambientes Oracle/PT-BR)
alter session set nls_numeric_characters='.,';

-- CTE: Parametros
-- Objetivo: Definir os filtros de busca conforme o padrao do arquivo NFe.sql
with parametros as (
    select 
        :CNPJ as cnpj_filtro,          
...
-- CTE: Filtro de Chaves
-- Objetivo: Identificar na tabela fato quais notas atendem aos filtros antes de processar o XML
-- Isso otimiza a consulta evitando full-scan na tabela de XML
filtro_chaves as (
    select distinct 
        d.chave_acesso
    from bi.fato_nfe_detalhe d,
         parametros p
    where 
        -- Filtro de Data (Emissao ou Saida/Entrada)
        GREATEST(COALESCE(d.dhsaient, d.dhemi), d.dhemi) BETWEEN p.data_inicial AND p.data_final
        -- Filtro de CNPJ (Destinatario ou Emitente)
        AND (d.co_destinatario = p.cnpj_filtro OR d.co_emitente = p.cnpj_filtro)
        -- Filtro de Status (Apenas notas autorizadas/canceladas validas no DW)
        AND d.INFPROT_CSTAT in (100, 150)
),

-- CTE 'portalfiscal'
-- Objetivo: Ler o XML bruto da tabela 'bi.nfe_xml' e transformá-lo em colunas relacionais
-- Agora filtrando apenas pelas chaves pré-selecionadas
portalfiscal as (
  select a.chave_acesso,
         b.*
  from bi.nfe_xml a
  -- Join com o filtro para limitar o processamento do XML
  inner join filtro_chaves f on a.chave_acesso = f.chave_acesso,

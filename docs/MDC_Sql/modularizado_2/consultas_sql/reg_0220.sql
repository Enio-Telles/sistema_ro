...
ARQUIVOS_RANKING AS (
    ...
    WHERE 
        reg_0000.data_entrega <= p.dt_corte -- Filtro de "Viagem no Tempo": Ignora retifica��es feitas ap�s a data de corte
        AND reg_0000.dt_ini BETWEEN p.dt_ini_filtro AND p.dt_fim_filtro
)
SELECT
    TO_CHAR(arq.dt_ini, 'MM/YYYY') AS periodo_efd,
    
    -- CORRE��O: Sintaxe correta para trazer todas as colunas de uma tabela/alias
    reg_0220.*,
    
    arq.cod_fin_efd,
    arq.data_entrega AS data_entrega_efd_periodo
    
FROM sped.reg_0220 reg_0220
INNER JOIN ARQUIVOS_RANKING arq 
  ON reg_0220.reg_0000_id = arq.reg_0000_id 
INNER JOIN sped.reg_0200 reg_0200 
    ON reg_0200.id = reg_0220.reg_0200_id

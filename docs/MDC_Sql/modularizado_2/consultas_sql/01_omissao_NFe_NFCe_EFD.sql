...
docs_saida AS (
    /* Passo 1: Busca NF-e e NFC-e de SAÍDA emitidas pelo próprio contribuinte */
    SELECT 
    ...
    WHERE d.dhemi >= p.data1 AND d.dhemi <= p.data2
...
efd_saidas AS (
    /* Passo 2: Chaves de SAÍDA escrituradas na EFD (Bloco C) */
...
notas_omissas AS (
    /* Passo 3: Identificação de Omissão via Left Join */
    SELECT d.*
    FROM docs_saida d 
...
max_evento AS (
    /* Passo 4 e 5: Consolida o último evento da nota */
    SELECT 
        ev.chave_acesso,
...
dados_pendencias AS (
    /* Passo 7: Busca os detalhes da malha na tabela de pendências */
    SELECT 
...
dados_malha AS (
    /* Passo 8: Busca o cadastro/título da malha */
    SELECT 
        m.id, 
        m.titulo
...
dados_notificacao AS (
    /* NOVO Passo 9: Busca os detalhes da notificação enviada para a pendência */
    SELECT 

/*
===============================================================================
SCRIPT: RESSARCIMENTO DE ICMS-ST - VERSÃO INTEGRAL REESCRITA (V4)
-------------------------------------------------------------------------------
Objetivo:
...
3) prioriza XML para inferência de NCM/CEST;
4) calcula sempre o valor calculado;
5) tenta primeiro vincular por cod_item limpo (sem conflito de NCM/CEST);
6) o score da v4 considera:
   - cod_item
   - GTIN/código de barras válido
   - NCM
   - CEST
   - combinação NCM + CEST
   - descrição normalizada
   - compatibilidade de quantidade
   - existência do item no XML como evidência auxiliar
7) conflitos de NCM/CEST geram penalidade explícita;
8) o ranking mede ambiguidade com score do segundo colocado (gap_top2);
9) RESSARC_ICMS_Proprio_Considerado = valor com base na nota fiscal;
10) RESSARC_ST_Considerado = prefere Fronteira; se Fronteira não encontrada
    ou rotina diferente de ST, usa CALC_VL_RESSARCIMENTO_ST.
...
/* ============================================================================
   8) XML DE ENTRADA
   item_xml_padrao = COALESCE(prod_nitem, seq_nitem)
   ============================================================================ */
XML_ENTRADA_BASE AS (
    SELECT
...
/* ============================================================================
   11) CANDIDATOS DE VÍNCULO - BASE (V4)
   Regras da v4:
   - cod_item entra no score e segue como prioridade apenas quando não há conflito fiscal;
   - alinhamento documental entre num_item do C170 e item do XML (PROD_NITEM/SEQ_NITEM) passa a compor o score;
   - GTIN/código de barras válido passa a compor o score;
   - mera existência do item no XML vira evidência auxiliar, e não dominante;
   - conflitos de NCM/CEST geram penalidade explícita;
   - descrição passa por normalização antes de comparação;
   ============================================================================ */
CANDIDATOS_VINCULO_ENTRADA_BASE AS (
    SELECT
        ...
        GREATEST(
            0,
            NVL(m.score_cod_item, 0)
          + NVL(m.score_num_item_doc, 0)
          + NVL(m.score_cod_barra, 0)
          + NVL(m.score_ncm, 0)
          + NVL(m.score_cest, 0)
          + NVL(m.score_ncm_cest_combo, 0)
          + NVL(m.score_descricao, 0)
          + NVL(m.score_quantidade, 0)
          + NVL(m.score_xml, 0)
          - NVL(m.penalidade_total, 0)
        ) AS score_vinculo_entrada,

        ABS(
            NVL(COALESCE(m.qcom_entrada, m.qtd_item_entrada_sped), 0) -
            NVL(m.qtd_saida_sped, 0)
        ) AS diff_qtd_vinculo
    FROM (
        SELECT
            r.*,
            ...
            CASE
                WHEN r.ind_match_cod_item = 1 THEN 35
                ELSE 0
            END AS score_cod_item,

            CASE
                WHEN r.ind_match_num_item_doc = 1 THEN 8
                ELSE 0
            END AS score_num_item_doc,

            CASE
                WHEN r.ind_match_cod_barra = 1 THEN 25
                ELSE 0
            END AS score_cod_barra,
            ...
            CASE
                ...
                WHEN r.desc_candidato_norm IS NOT NULL
                 ...
                 AND (
                        r.desc_candidato_norm LIKE '%' || r.desc_saida_sped_norm || '%'
                ...
                WHEN r.desc_candidato_norm IS NOT NULL
                 ...
                 AND (
                        r.desc_candidato_norm LIKE '%' || r.desc_saida_0200_norm || '%'
                     OR r.desc_saida_0200_norm LIKE '%' || r.desc_candidato_norm || '%'
                 )
                THEN 4
                ELSE 0
            END AS score_descricao,
            ...
            CASE
                ...
                WHEN NVL(r.qtd_saida_sped, 0) > 0
                 AND ABS(
                        NVL(COALESCE(r.qcom_entrada, r.qtd_item_entrada_sped), 0) -
                        NVL(r.qtd_saida_sped, 0)
                     ) / r.qtd_saida_sped <= 0.20
                THEN 2
                ELSE 0
            END AS score_quantidade,

            CASE
                WHEN r.item_xml_entrada_padrao IS NOT NULL THEN 3
                ELSE 0
            END AS score_xml,

            CASE
                WHEN r.ind_conflito_ncm = 1 THEN 20
                ELSE 0
            END AS penalidade_ncm,

            CASE
                WHEN r.ind_conflito_cest = 1 THEN 15
                ELSE 0
            END AS penalidade_cest,
                ...
                NULLIF(
                    TRIM(
                        REGEXP_REPLACE(
                            REGEXP_REPLACE(
                                TRANSLATE(
                                    UPPER(COALESCE(s.descricao_item, '')),
                                    'ÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇÑ',
                                    'AAAAAEEEEIIIIOOOOOUUUUCN'
                                ),
                                '[^A-Z0-9 ]',
...
/* ============================================================================
   ...
   Regra:
   - cod_item limpo (sem conflito de NCM/CEST) recebe prioridade;
   - na ausência disso, vence o maior score;
   - empate técnico é medido por score do segundo colocado.
   ============================================================================ */
...
VINCULO_ENTRADA_ESCOLHIDO AS (
    SELECT
        ...
        score_segundo_colocado,
        score_vinculo_entrada - NVL(score_segundo_colocado, 0) AS gap_top2,
        diff_qtd_vinculo,
        CASE
            WHEN ind_match_cod_item = 1
             AND ind_conflito_ncm = 0
             AND ind_conflito_cest = 0
                THEN 'VINCULO DETERMINISTICO POR COD_ITEM'
            WHEN ind_match_cod_item = 1
             AND (ind_conflito_ncm = 1 OR ind_conflito_cest = 1)
        ...
        CASE
            WHEN score_vinculo_entrada - NVL(score_segundo_colocado, 0) < 8
...
SELECT
    ...
    b.score_cod_item                    AS aud_score_cod_item,
    b.score_num_item_doc                AS aud_score_num_item_doc,
    b.score_cod_barra                   AS aud_score_cod_barra,
    b.score_ncm                         AS aud_score_ncm,
    b.score_cest                        AS aud_score_cest,
    b.score_ncm_cest_combo              AS aud_score_ncm_cest_combo,
    b.score_descricao                   AS aud_score_descricao,
    b.score_quantidade                  AS aud_score_quantidade,
    b.score_xml                         AS aud_score_xml,
    b.penalidade_ncm                    AS aud_penalidade_ncm,
    b.penalidade_cest                   AS aud_penalidade_cest,
    b.penalidade_total                  AS aud_penalidade_total,

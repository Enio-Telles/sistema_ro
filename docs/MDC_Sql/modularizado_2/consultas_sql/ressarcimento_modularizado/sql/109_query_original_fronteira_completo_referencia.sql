/* ===================================================================================
   CONSULTA DE ANLISE DE FRONTEIRA / SITAFE (VERSO OTIMIZADA - SEFIN/RO)
   ===================================================================================
   Objetivo: Cruzar dados de Notas Fiscais Eletrnicas (NFe) do BI com os lanamentos
   contbeis e fiscais do sistema SITAFE.
   
   Otimizaes (Auditoria):
   1. Remoo de DISTINCT oneroso e substituio por GROUP BY na Subquery ORIGEM.
   2. Adequao SARGable: Filtros de data convertidos para string (YYYYMMDD) para 
      permitir o uso de ndices B-Tree na tabela sitafe_nota_fiscal.
   3. Eliminao de CROSS JOIN em subqueries para evitar Cartesian Plans.
   4. Reordenao dos Joins priorizando a Fato NFe filtrada como Driving Table.
=================================================================================== */
...
ORIGEM_NFE AS (
    ...
    INNER JOIN PARAMETROS P ON 1=1 -- Fora a amarrao das variveis sem Cross Join solto
    WHERE 
...
SELECT 
      -- ==============================================================================
      -- DADOS DE IDENTIFICAO DA NOTA E EMITENTE
      -- ==============================================================================
      origem.chave,                                                                                                     
      origem.nota,                                                                                                      
      origem.cnpj_emit,
      ...
      -- ==============================================================================
      -- VALORES MONETRIOS DO ITEM (Custos e Impostos)
      -- ==============================================================================
      ...
      -- CLCULO: Valor Total Lquido do Produto
      ...
      -- ==============================================================================
      -- DADOS DO LANAMENTO E PAGAMENTO
      -- ==============================================================================
      ...
      CASE 
            ...
            WHEN lanc.it_co_situacao_lancamento = '13' THEN 'CORREO NO PAGAMENTO ORIGINAL'
            ...
            WHEN lanc.it_co_situacao_lancamento = '10' THEN 'BAIXA PROVISRIA'
            WHEN lanc.it_co_situacao_lancamento = '14' THEN 'LANAMENTO EXCLUDO'
            WHEN lanc.it_co_situacao_lancamento = '32' THEN 'COMPENSAO'
            WHEN lanc.it_co_situacao_lancamento = '38' THEN 'LIQUIDAO DESVINCULADA DE CONTA GRFICA'
            WHEN lanc.it_co_situacao_lancamento = '46' THEN 'SUSPENSO JUDICIAL'
            WHEN lanc.it_co_situacao_lancamento = '50' THEN 'LANAMENTO INDEVIDO'
            WHEN lanc.it_co_situacao_lancamento IS NULL THEN ' '
            ELSE 'VERIFICAR' 
      END situao,

      -- ==============================================================================
      -- DETALHAMENTO TRIBUTRIO ESPECFICO (Mercadoria/Sefin)
      -- ==============================================================================
...
FROM ORIGEM_NFE origem
    ...
    -- Busca os parmetros para garantir os filtros SARGable (via INNER JOIN 1=1 ou CROSS JOIN seguro)
    INNER JOIN PARAMETROS p ON 1=1
        
    -- Explode para o Lanamento (Nota)
...
WHERE
    -- Filtros aplicados de forma SARGable (coluna isolada, sem funes)

/* =========================================================================================
   AUDITORIA EFD x DOCUMENTOS ELETRNICOS (MODELOS 55, 65 E 57) - POR PERODO
   =========================================================================================
   Objetivo:
   - Confrontar documentos eletrnicos autorizados no BI (NF-e, NFC-e e CT-e)
     com os documentos escriturados na EFD;
   - Identificar omisses de escriturao;
   - Identificar divergncias de ICMS entre XML/BI e EFD;
   - Identificar escriturao em perodo diferente da emisso;
   - Enriquecer omisses de entrada com eventos de manifestao do destinatrio;
   - Identificar documentos escriturados na EFD sem correspondente localizado no BI/XML.

   Observaes:
   - A consulta preserva a inteno fiscal da verso original.
   - Foram corrigidos pontos frgeis de robustez e consistncia tcnica.
========================================================================================= */
...
/* =========================================================================================
   1) PARMETROS
   -----------------------------------------------------------------------------------------
   Centraliza os parmetros e padroniza:
   - CNPJ sem mscara;
   - datas inicial e final como DATE;
   - valores padro para evitar dependncia de converses implcitas.
========================================================================================= */
parametros AS (
    SELECT
...
/* =========================================================================================
   ...
   - identificar CT-e em que o contribuinte  tomador;
   - identificar CT-e em que o contribuinte  o emitente.
========================================================================================= */
cte_ajuste AS (
    SELECT
        c.chave_acesso,
...
/* =========================================================================================
   ...
   -----------------------------------------------------------------------------------------
   Consolida os documentos eletrnicos autorizados no BI, por papel do contribuinte:
   - Entrada de NF-e;
   - Entrada prpria;
...
docs AS (
    ...
    /* -------------------------------------------------------------------------------------
       3.2) ENTRADA PRPRIA
       Mantida conforme a lgica original da auditoria.
    ------------------------------------------------------------------------------------- */
    SELECT
        d.infprot_cstat AS status,
    ...
    /* -------------------------------------------------------------------------------------
       ...
       Mantido exatamente conforme a lgica operacional original.
    ------------------------------------------------------------------------------------- */
    SELECT
        d.infprot_cstat AS status,
    ...
    /* -------------------------------------------------------------------------------------
       3.6) CT-e (MODELO 57)
       - Tomador 57: contribuinte  tomador e no  emitente;
       - Saida 57  : contribuinte  emitente.
    ------------------------------------------------------------------------------------- */
    SELECT
...
/* =========================================================================================
   ...
   - C100 (NF-e / NFC-e);
   - D100 (CT-e).
   Usa apenas arquivos considerados vlidos pela dimenso BI.DM_EFD_ARQUIVO_VALIDO.
========================================================================================= */
efd AS (
...
/* =========================================================================================
   5) CRUZAMENTO PRINCIPAL: DOCUMENTO DO BI/XML x ESCRITURAO EFD
   -----------------------------------------------------------------------------------------
   Chave principal do confronto: chave de acesso.
   Resultado:
   - se existe na EFD;
   - em qual perodo foi escriturado;
   - se h diferena de ICMS;
   - se o perodo da escriturao coincide com o da emisso.
========================================================================================= */
base AS (
    SELECT
        d.status,
...
/* =========================================================================================
   6) OMISSES DE ENTRADA
   -----------------------------------------------------------------------------------------
   Isola as entradas que existem no BI/XML, mas no foram localizadas na EFD.
========================================================================================= */
omissao_entrada AS (
    SELECT
        b.chave_acesso
...
/* =========================================================================================
   7) EVENTOS DE MANIFESTAO DO DESTINATRIO
   -----------------------------------------------------------------------------------------
   Enriquecimento das omisses de entrada com eventos da NF-e:
   - cincia da operao;
   - confirmao;
   - desconhecimento;
   - operao no realizada;
...
/* =========================================================================================
   8) LTIMO EVENTO POR NOTA
   -----------------------------------------------------------------------------------------
   Seleciona o evento mais recente da manifestao, usando o maior NSU.
========================================================================================= */
max_ev_nota AS (
    SELECT
        e.chave_acesso,
...
/* =========================================================================================
   ...
   documentos que esto na EFD, mas no foram localizados no conjunto principal "docs".
   Aqui o lookup  simtrico e cobre:
   - NF-e;
   - NFC-e;
   - CT-e.
========================================================================================= */
...
/* =========================================================================================
   ...
   -----------------------------------------------------------------------------------------
   Este  o cruzamento reverso:
   - parte da EFD;
   - procura a chave no conjunto documental principal do BI/XML;
   - se no encontra, sinaliza como "EFD no cruzada".
========================================================================================= */
docs_n_cruzados AS (
    SELECT
        TO_CHAR(e.efd_ref, 'YYYY/MM') AS efd_ref,

...
O MDC passou a incluir uma camada explícita para diagnosticar quando a comparação de quantidades precisa de conversão de unidade. Isso é especialmente relevante nas trilhas de ressarcimento (`C176`) e de inventário / mudança de tributação (`H010` x última entrada), onde a comparação direta de quantidade e o cálculo de valor unitário deixam de ser confiáveis quando há divergência entre `unid`, `unid_inv`, `unid_conv`, `qcom` ou `it_un_comercial`.

Arquivo novo:
- `sql_mdc/24_diagnostico_necessidade_conversao_unidade.sql`

Documento explicativo:
- `conversao_unidades.md`

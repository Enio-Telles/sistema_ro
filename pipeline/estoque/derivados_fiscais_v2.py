...
def _mva_efetivo_expr() -> pl.Expr:
    # Implementação simplificada: quando houver MVA ajustado, usa a mesma base percentual já trazida.
    return pl.when(pl.col("it_pc_mva") > 0).then(pl.col("it_pc_mva") / 100.0).otherwise(1.0)


def build_aba_mensal_v2(mov_df: pl.DataFrame) -> pl.DataFrame:
    if mov_df.is_empty():
        return pl.DataFrame()
    ...
    result = df.group_by(["id_agrupado", "ano", "mes"]).agg(
        ...
        .then(
            ...
            .otherwise(pl.col("pme_mes") * pl.col("entradas_desacob") * (pl.col("aliq_interna").fill_null(0.0) / 100.0) * _mva_efetivo_expr())
        )
        .otherwise(0.0)
        ...
        .then(
            ...
            .otherwise(pl.col("pme_mes") * pl.col("entradas_desacob_periodo") * (pl.col("aliq_interna").fill_null(0.0) / 100.0) * _mva_efetivo_expr())
        )
        .otherwise(0.0)
...
def build_aba_anual_v2(mov_df: pl.DataFrame) -> pl.DataFrame:
    if mov_df.is_empty():
        return pl.DataFrame()
...
def build_aba_periodos_v2(mov_df: pl.DataFrame) -> pl.DataFrame:

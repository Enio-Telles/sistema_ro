...
def _ensure_text_cols(df: pl.DataFrame) -> pl.DataFrame:
    result = df
    for col in MANDATORY_TEXT_COLS:
        if col in result.columns:
            result = result.with_columns(pl.col(col).cast(pl.Utf8, strict=False).fill_null("").alias(col))
        else:
...
def _normalize_source_rows(df: pl.DataFrame, *, source_prefix: str) -> pl.DataFrame:
    if df.is_empty():
        return pl.DataFrame(schema={col: pl.Utf8 for col in MANDATORY_TEXT_COLS})

    result = _ensure_text_cols(df.with_row_index("__row_nr__"))
    result = result.with_columns(
        pl.when(pl.col("id_linha_origem") == "")
        .then(pl.concat_str([pl.lit(source_prefix), pl.lit("|"), pl.col("codigo_produto_original"), pl.lit("|"), pl.col("__row_nr__").cast(pl.Utf8)]))
        .otherwise(pl.col("id_linha_origem"))
        .alias("id_linha_origem"),
        pl.when(pl.col("codigo_fonte") == "")
        .then(pl.concat_str([pl.col("cnpj"), pl.lit("|"), pl.col("codigo_produto_original")]))
        .otherwise(pl.col("codigo_fonte"))
        .alias("codigo_fonte"),
    )
    return result.drop("__row_nr__")


def _build_map_lookup(map_produto_agrupado_df: pl.DataFrame) -> pl.DataFrame:
    if map_produto_agrupado_df.is_empty() or "codigo_fonte" not in map_produto_agrupado_df.columns:
        return pl.DataFrame()
    keep = [col for col in ["codigo_fonte", "id_agrupado", "descricao_normalizada"] if col in map_produto_agrupado_df.columns]
    return map_produto_agrupado_df.select(keep).unique(subset=["codigo_fonte"])
...
def _enrich_with_agregacao(df: pl.DataFrame, *, map_lookup_df: pl.DataFrame, produtos_lookup_df: pl.DataFrame) -> tuple[pl.DataFrame, pl.DataFrame]:
    ...
    if not produtos_lookup_df.is_empty() and "id_agrupado" in result.columns:
        prod_lookup = produtos_lookup_df.rename({"gtin_padrao": "gtin_padrao_agr"}) if "gtin_padrao" in produtos_lookup_df.columns else produtos_lookup_df
        result = result.join(prod_lookup, on="id_agrupado", how="left")

    if "id_agrupado" not in result.columns:
        result = result.with_columns(pl.lit("").alias("id_agrupado"))
    for col in ["descr_padrao", "ncm_padrao", "cest_padrao", "gtin_padrao_agr", "unid_ref"]:
...
def build_fontes_agr_v2(
    *,
    ...
    c170_ok, c170_sem = _enrich_with_agregacao(_normalize_source_rows(c170_df, source_prefix="c170"), map_lookup_df=map_lookup_df, produtos_lookup_df=produtos_lookup_df)
    nfe_ok, nfe_sem = _enrich_with_agregacao(_normalize_source_rows(nfe_df, source_prefix="nfe"), map_lookup_df=map_lookup_df, produtos_lookup_df=produtos_lookup_df)
    nfce_ok, nfce_sem = _enrich_with_agregacao(_normalize_source_rows(nfce_df, source_prefix="nfce"), map_lookup_df=map_lookup_df, produtos_lookup_df=produtos_lookup_df)
    bloco_h_ok, bloco_h_sem = _enrich_with_agregacao(_normalize_source_rows(bloco_h_df, source_prefix="bloco_h"), map_lookup_df=map_lookup_df, produtos_lookup_df=produtos_lookup_df)

...
from pipeline.estoque.derivados_fiscais_v3 import build_aba_anual_v3, build_aba_mensal_v3, build_aba_periodos_v3
...
from pipeline.mercadorias.pipeline_v2 import run_mercadoria_pipeline_v2


def run_gold_pipeline_v7(
    ...
    *,
    ...
    mercadorias = run_mercadoria_pipeline_v2(itens_df, base_info_df=base_info_df, mapa_manual_df=mapa_manual_df)
    produtos_final = mercadorias["produtos_final"]
    item_unidades = build_item_unidades_v2(itens_df, produtos_final)
    fatores = calcular_fatores_priorizados(item_unidades, itens_df)
    ...
    aba_mensal = build_aba_mensal_v3(mov_estoque)
    aba_anual = build_aba_anual_v3(mov_estoque)
    aba_periodos = build_aba_periodos_v3(mov_estoque)
    estoque_resumo = build_estoque_resumo(aba_anual, fatores)
    estoque_alertas = build_estoque_alertas(aba_anual, fatores)
    return {
        **mercadorias,
        "mapa_agrupamento_manual": mapa_manual_df if mapa_manual_df is not None else pl.DataFrame(),

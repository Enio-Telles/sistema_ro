...
def execute_gold_v20(cnpj: str) -> dict:
    ...
    result["diagnostico_conversao_rows"] = 0 if inputs["diagnostico_conversao_df"].is_empty() else inputs["diagnostico_conversao_df"].height
    result["warnings"] = []
    ...
    if not inputs["diagnostico_conversao_df"].is_empty():
        result["warnings"].append("Diagnóstico de conversão de unidades integrado ao cálculo de item_unidades e fatores.")
    ...
    if not consistency.get("ok", False):
        result["warnings"].append("Inconsistência detectada entre mov_estoque e abas fiscais derivadas após a execução.")
    return result
